...
const crawlPage = async (pageUrl, baseUrl, progressBar) => {
  ...
  try {
    await page.goto(pageUrl, { waitUntil: 'networkidle2', timeout: 60000 }); // Increased timeout to 60 seconds
  } catch (error) {
    ...
    if (href) {
      ...
      if (urlObject.origin === baseUrl.origin) {
        if (absoluteUrl.endsWith('.pdf')) {
          ...
          limit(() => crawlPage(absoluteUrl, baseUrl, progressBar));
        }
      }
    }
  });
...
const main = async (startUrl) => {
  ...
  const progressBar = new cliProgress.MultiBar({
    ...
    hideCursor: true,
    format: 'Progress [{bar}] {percentage}% | ETA: {eta}s | {value}/{total} Pages'
  }, cliProgress.Presets.shades_grey);

  const crawlPageWithProgress = (pageUrl) => {
    const bar = progressBar.create(1, 0); // Initial progress set to 0 out of 1
    return crawlPage(pageUrl, baseUrl, bar);
  };

  await limit(() => crawlPageWithProgress(startUrl));
  progressBar.stop();
